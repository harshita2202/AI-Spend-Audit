...
export function ToolCard({ tool, value, onChange }) {
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-3">
              ...
              >
                {tool.plans.map((plan) => (
                  <option key={plan.label} value={plan.label}>
                    {plan.label}
                    {plan.pricePerSeat !== null && !plan.custom ? ` · $${plan.pricePerSeat}/seat` : plan.custom ? " · custom" : ""}
                  </option>
                ))}
              </select>
            </div>
            ...
            <div>
              ...
              <div className="relative">
                <span className="absolute left-2.5 top-1/2 -translate-y-1/2 text-muted-fg text-sm">$</span>
                <input
                  type="number"
                  min={0}
